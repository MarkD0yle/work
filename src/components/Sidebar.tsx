...
export default function Sidebar({
  ...
  // Recent pages: resolve slugs in visit order, filter by search query.
  const recentPages = useMemo<PageEntry[]>(() => {
    return recentSlugs
      .map((slug) => pages.find((p) => p.slug === slug))
      .filter(
        (p): p is PageEntry =>
          p !== undefined &&
          (!searching || p.title.toLowerCase().includes(trimmedQuery)),
      );
  }, [recentSlugs, pages, searching, trimmedQuery]);

  // Section groups — only used in "all" mode.
    ...
    >
      ...
      {!collapsed && (
        <div className="border-b border-neutral-200 px-3 py-2.5">
          {/* All / Recent toggle */}
          <div className="mb-2.5 flex items-center gap-1 rounded-md bg-neutral-100 p-0.5">
            <button
              type="button"
              onClick={() => setViewMode("all")}
              className={`flex-1 rounded-sm px-2 py-1 text-[11px] font-medium transition ${
                viewMode === "all"
            ...
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-1 rounded-sm px-2 py-1 text-[11px] font-medium transition ${
                viewMode === "recent"
                  ? "bg-white text-neutral-900 shadow-sm"
                  : "text-neutral-500 hover:text-neutral-800"
              }`}
          ...
          {/* Section filters — only relevant in All mode */}
          {viewMode === "all" && (
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        {viewMode === "recent" ? (
          /* ── Recent view: flat list in visit order ── */
          recentPages.length === 0 ? (
            !collapsed && (
              <div className="px-4 py-6 text-center text-xs text-neutral-400">
                {searching ? `No recent pages match "${trimmedQuery}".` : "No pages visited yet."}
              </div>
            )
          ) : (
      ...
      {!collapsed && (
        <div className="border-t border-neutral-200 p-3 text-[11px] text-neutral-400">
          {viewMode === "recent" ? (
            <span>
              {recentPages.length} recently visited
            </span>
