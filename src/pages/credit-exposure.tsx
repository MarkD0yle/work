...
/* ------------------------------------------------------------------ *
 * Counterparty row — clean, one main statement per row.
 * ------------------------------------------------------------------ */
...
function CounterpartyRow({
  ...
  const tone = TONE[w];
  const sr = cp.limits.find((l) => l.key === "sr")!;
  const u = utilisation(sr);
  const cdsTone =
    cp.cds5yChange1d > 0
      ? "stroke-rose-500"
      : cp.cds5yChange1d < 0
  ...
  return (
    <button
      ...
      className={`relative flex w-full items-stretch border-b border-neutral-100 text-left transition ${
        selected ? "bg-neutral-50" : "hover:bg-neutral-50/60"
      }`}
    >
      <span
        aria-hidden
        className={`block w-1 shrink-0 ${selected ? "bg-neutral-900" : tone.bar}`}
      />
      <div className="flex-1 px-3 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {cp.watchlist && (
              <span className="rounded bg-amber-100 px-1 py-0.5 text-[9px] font-semibold tracking-wide uppercase text-amber-800">
                Watch
              </span>
            )}
          </div>
          <span className={`text-[10px] font-semibold ${tone.text}`}>
            {w === "ok" ? "Clean" : w === "warn" ? "Soft" : "Breach"}
          </span>
        </div>
